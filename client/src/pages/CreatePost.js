...
function CreatePost() {
  ...
  const handleCreatePost = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/api/posts', { title, content, category }, {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg mt-7">
      <h2 className="text-3xl font-semibold text-gray-800 mb-6">Crear Post</h2>
      <form onSubmit={handleCreatePost} className="space-y-6">
        ...
        <div className="mt-4">
          ...
          <button
            ...
            className={`w-full bg-green-500 text-white py-3 rounded-lg hover:bg-green-600 transition duration-200 mt-2 ${loading && 'opacity-50 cursor-not-allowed'}`}
            disabled={loading}
          >
        ...
        <button
          type="submit"
          className={`w-full bg-indigo-500 text-white py-3 rounded-lg hover:bg-indigo-600 transition duration-200 ${loading && 'opacity-50 cursor-not-allowed'}`}
          disabled={loading}
        >
